...
export default function TemplateLivePreview({
  ...
  return (
    <div className="surface flex h-full min-h-0 flex-col p-4">
      <p className="shrink-0 text-2xs font-semibold uppercase tracking-[0.08em] text-ink-500 dark:text-ink-400">
        Live preview
      </p>
      <p className="mt-1 shrink-0 truncate text-sm font-medium text-ink-900 dark:text-ink-100">
        {subject || <span className="italic text-ink-400">(no subject)</span>}
      </p>
      {hint ? (
        <p className="mt-0.5 shrink-0 text-2xs text-ink-400 dark:text-ink-500">{hint}</p>
      ) : null}
      <div className="mt-3 flex min-h-0 flex-1 flex-col">
        {empty ? (
          <div className="grid flex-1 place-items-center rounded-md border border-dashed border-ink-300 bg-white/60 px-3 py-6 text-center text-xs text-ink-400 dark:border-ink-700 dark:bg-ink-900/40 dark:text-ink-500">
            {emptyMessage}
          </div>
        ) : (
          <iframe
            ...
            sandbox=""
            className="preview-frame min-h-0 w-full flex-1 rounded-md border border-ink-200/70 bg-white dark:border-ink-700"
          />
        )}
      </div>
    </div>
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="grid min-h-0 flex-1 grid-cols-1 lg:grid-cols-[minmax(0,1fr)_minmax(440px,50%)] lg:gap-0">
        ...
        <aside className="flex min-h-[min(50vh,420px)] flex-col border-t border-ink-200/70 pt-4 dark:border-ink-800 lg:min-h-0 lg:border-t-0 lg:border-l lg:pt-0 lg:pl-5 dark:lg:border-ink-800">
          {preview}
        </aside>
      </div>
    </div>
